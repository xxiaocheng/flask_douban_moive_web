...
class CelebrityInfo(Resource):
    ...
    def get(self, id):
        ...
        parser.add_argument('cate', default='detail', choices=[
                            'summary', 'detail'], location='args')
...
class AddCelebrity(Resource):
    ...
    def post(self):
        ...
        parser.add_argument('gender', required=True, choices=[
                            '男', '女'], location='form')
        parser.add_argument('name_en', default='', type=str, location='form')
        parser.add_argument('born_place', default='',
        ...
        with open(os.path.join(current_app.config['CELEBRITY_IMAGE_UPLOAD_PATH'], avatar_filename), 'wb') as f:
            args['avatar'].save(f)
        ...
        Celebrity(douban_id=args.douban_id, avatar=avatar_filename, name=args.name, gender=args.gender,
                  name_en=args.name_en, born_place=args.born_place, aka=aka, aka_en=aka_en).save()
        return{
            'message': '艺人信息添加成功'
        }
