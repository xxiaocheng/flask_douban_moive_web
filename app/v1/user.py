...
class UserFriends(Resource):
    ...
    def get(self, username, follower_or_following):
        ...
        if follower_or_following.lower() == 'follower':
            ...
            items = [user_summary_schema(follow.follower)
        ...
        if follower_or_following.lower() == 'following':
            pagination = Follow.objects(follower=user, is_deleted=False).paginate(
                page=args['page'], per_page=args['per_page'])
            items = [user_summary_schema(follow.followed)
                     for follow in pagination.items]

        prev = None
        if pagination.has_prev:
            prev = url_for('.userfriends', username=username, follower_or_following=follower_or_following,
                           page=args['page']-1, per_page=args['per_page'], _external=True)

        next = None
        if pagination.has_next:
            next = url_for('.userfriends', username=username, follower_or_following=follower_or_following,
                           page=args['page']+1, per_page=args['per_page'], _external=True)

        first = url_for('.userfriends', username=username, follower_or_following=follower_or_following,
                        page=1, per_page=args['per_page'], _external=True)
        last = url_for('.userfriends', username=username, follower_or_following=follower_or_following,
