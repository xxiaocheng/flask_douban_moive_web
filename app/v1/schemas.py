...
def user_summary_schema(user):
    return {
        ...
        "avatar": url_for(
            "api.photo", cate="avatar", filename=user.avatar_l, _external=True
        ),
...
def movie_summary_schema(movie):
    return {
        ...
        "image": url_for(
            "api.photo", cate="movie", filename=movie.image, _external=True
        ),
        "score": movie.score,
...
def celebrity_summary_schema(celebrity):
    return {
        ...
        "name": celebrity.name,
        "image": url_for(
            "api.photo", cate="celebrity", filename=celebrity.avatar, _external=True
        ),
...
def movie_schema(movie):
    ...
    try:
        directors = [
            celebrity_summary_schema(celebrity)
            for celebrity in movie.directors
            if movie.directors and celebrity.is_deleted == False
        ]
    except:
        directors = []
    try:
        casts = [
            celebrity_summary_schema(cast)
            for cast in movie.casts
            if movie.casts and cast.is_deleted == False
        ]
    except:
        casts = []
    user = g.current_user
...
def celebrity_schema(celebrity):
    return {
        ...
        "direct_movies": [
            ...
            for movie in Movie.objects(directors__in=[celebrity])
            if movie.is_deleted == False
        ],
        "casts_movies": [
            movie_summary_schema(movie)
            for movie in Movie.objects(casts__in=[celebrity])
            if movie.is_deleted == False
        ],
    }
...
def rating_schema(rating):
    schema = {
        ...
        "useravatar": url_for(
            "api.photo", cate="avatar", filename=rating.user.avatar_l, _external=True
        ),
        "id": str(rating.id),
        "comment": rating.comment,
        "me2rating": "unlike",
...
def _like_schema(like):
    return {
        ...
        "who_avatar": url_for(
            "api.photo", cate="avatar", filename=like.user.avatar_l, _external=True
        ),
...
def _follow_schema(follow):
    return {
        ...
        "who_avatar": url_for(
            "api.photo",
            cate="avatar",
            filename=follow.follower.avatar_l,
            _external=True,
        ),
    }
