...
def send_confirm_email(token, to, username):
    ...
    response = _send_email(subject='确认邮箱', to=to,
                           html_body=render_template('emails/confirm.html', username=username, url_to=url_to))
...
def send_reset_password_email(token, to, username):
    ...
    response = _send_email(subject='请重置密码', to=to,
                           html_body=render_template('emails/reset_password.html', username=username, url_to=url_to))
    return response.status_code
...
def send_change_email_email(token, to, username):
    ...
    response = _send_email(subject='更改绑定邮箱', to=to,
                           html_body=render_template('emails/change_email.html', username=username, url_to=url_to))
    return response.status_code
