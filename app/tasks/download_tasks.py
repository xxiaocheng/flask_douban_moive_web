...
def sent_sys_message_to_user(user, message=None):
    ...
    notitication = Notification(receiver=user, category=2, system_info=message)
    notitication.save()
...
def sent_sys_message_to_admin(message=None):
    with redis_store.app.app_context():
        admin_user = User.objects(
            email=current_app.config["ADMIN_EMAIL"], is_deleted=False
        ).first()
    ...
    if cate == "coming":
        ...
        if subjects:
            for movie_obj in subjects:
                try:
                    if not redis_store.sismember(
                        "downloaded:movie", movie_obj.get("id")
                    ):
                        download_movie(movie_obj.get("id"))
                        time.sleep(10)
                    movie = Movie.objects(douban_id=movie_obj.get("id")).first()
                    if movie:
                        Cinema.objects(cate=1, movie=movie).update(
                            upsert=True, cate=1, movie=movie
                        )
                except:
                    error = True
        ...
        if subjects:
            for movie_obj in subjects:
                try:
                    ...
                    movie = Movie.objects(douban_id=movie_obj.get("id")).first()
                    if movie:
                        Cinema.objects(cate=0, movie=movie).update(
                            upsert=True, cate=0, movie=movie
                        )
                except:
                    error = True
...
def download_image_from_redis():
    ...
    while redis_store.zcard("task:image"):
        image_url = redis_store.zrange("task:image", 0, 0, desc=True)[0].decode()
...
def download_celebrity_from_redis():
    ...
    while redis_store.zcard("task:celebrity"):
        celebrity_id = redis_store.zrange("task:celebrity", 1, 1, desc=True)[0].decode()
        if not redis_store.sismember("downloaded:celebrity", celebrity_id):
            ...
            image_task = download_task(avatar_url, cate="image")  # 将下载头像的任务添加到redis队列中
            add_download_task_to_redis(image_task)
            Celebrity.objects(douban_id=celebrity_id).update(
                upsert=True,
                douban_id=celebrity_id,
                name=r_json.get("name", None),
                gender=r_json.get("gender", None),
                avatar=avatar,
                born_place=r_json.get("born_place"),
                aka_en=r_json.get("aka_en"),
                name_en=r_json.get("name_en"),
                aka=r_json.get("aka"),
                is_deleted=False,
...
def parse_movie_json(r_json):
    ...
    for genre in genres:
        Tag.objects(name=genre, cate=1).update(upsert=True, name=genre, cate=1)
    ...
    for cast in casts + directors:
        try:
            ...
            celebrity_task = download_task(
                cast.get("id"), "celebrity"
            )  # 将下载演员的任务添加到redis中
    ...
    Movie(
        douban_id=r_json["id"],
        title=r_json["title"],
        subtype=r_json["subtype"],
        year=int(r_json["year"]),
        image=image,
        seasons_count=r_json.get("seasons_count", None),
        episodes_count=r_json.get("episodes_count", None),
        countries=r_json.get("countries", None),
        genres=genres_obj,
        current_season=r_json.get("current_season", None),
        original_title=r_json["original_title"],
        summary=r_json["summary"],
        aka=r_json["aka"],
        directors=directors_obj,
        casts=casts_obj,
    ).save()
...
def get_douban_user_import_from_redis():
    ...
    while redis_store.exists("task:douban"):
        ...
        collect_list, wish_list, do_list = get_user_profile_movies(
            task_json["douban_id"]
        )
