...
def email_task(user, cate):
    assert cate in [Operations.CHANGE_EMAIL,
                    Operations.CONFIRM, Operations.RESET_PASSWORD]
    return{
        'to': user.email,
        'cate': cate,
...
def download_task(subjects_id, cate, priority=0):
    ...
    """
    assert cate in ['movie:showing', 'movie:coming',
                    'movie', 'celebrity', 'image']
    return{
...
def rank_redis_zset_paginate(keys, time_range='week', page=1, per_page=20, desc=True, withscores=False):
    ...
    """
    today_rank_key = 'rating:rank:'+time_range + \
