...
def query_by_id_list(document, id_list):
    ...
    """
    return [
        document.objects(id=id, is_deleted=False).first() for id in id_list if id_list
    ]
...
def validate_email_confirm_token(
    token, operation, user=None, new_password=None, new_email=None
):
